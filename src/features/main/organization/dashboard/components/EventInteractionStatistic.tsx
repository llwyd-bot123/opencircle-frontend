...
export default function EventInteractionStatistic() {
  ...
  const categories = useMemo(() => {
    ...
    const sorted = [...items].sort((a, b) => new Date(a.event_date).getTime() - new Date(b.event_date).getTime());
    return sorted.map((e) => (e.event_title.length > 8 ? `${e.event_title.slice(0, 8)}...` : e.event_title));
  }, [data?.event_analytics]);

  const series = useMemo(() => {
    const items = data?.event_analytics ?? [];
    const sorted = [...items].sort((a, b) => new Date(a.event_date).getTime() - new Date(b.event_date).getTime());
    const dataPoints = sorted.map((e) => e.comment_count);
  ...
  return (
    <section className="mb-6">
      <h2 className="text-responsive-base font-bold text-primary mb-3">Event Interaction</h2>
